...
typedef struct _label_t_
{
	char *name;
	uint16_t value;
	struct _label_t_ *next;
} _label_t_;

_label_t_ *label_list=NULL;

void Label_Add(const char *fname,uint16_t value)
{
_label_t_ *label = (_label_t_*)malloc(sizeof(_label_t_));

	label->name=strdup(fname);
	label->value=value;
...
void Labels_Dispose()
{
_label_t_ *list = label_list;

	while(list!=NULL)
	{
		_label_t_ *next = list->next;
		if (list->name!=NULL)
			free(list->name);
		free(list);
		list=next;
